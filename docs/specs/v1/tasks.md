...
- [x] T037 [REQ-006] profileUsage errors reach CLI output unredacted. Extract the runtime secret redaction into a shared module and apply it with the profile's access+refresh secrets. Regression: refresh error echoing credentials → CLI stderr and ProfileUsageResult.error carry no token. Verify: `bun test test/usage/fetch.test.ts test/cli/contract.test.ts` → GREEN
...
- [x] T042 [REQ-004, REQ-006] T037 pre-collects redaction secrets from an unlocked read: when the same-label credential is replaced before `ensureFreshProfile`'s locked re-read, the refresh error carries the NEW credential's secrets unredacted (`ProfileUsageResult.error`, and the coordinator's abort reason for the same reason). Generalize T034's actually-sent identity into a shared `RefreshOptions.onAttempt` observer — the only place that knows what was sent — and drive both fail-closed blocks and redaction from it; `InvalidGrantError.sentRefresh/sentAccess` retire into it. Regression: concurrent same-label replacement → neither surface's error output contains secret material. Verify: `bun test test/usage/fetch.test.ts test/extension/fail-closed.test.ts test/cli/contract.test.ts` → GREEN
