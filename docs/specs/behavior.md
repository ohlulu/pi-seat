...
| AC-029 | 任何 payload、任何寬度 | 以 `color: false` 渲染 | 輸出與 pace 上線前逐 byte 相同（AC-011a 的 golden fixtures 不變） |
