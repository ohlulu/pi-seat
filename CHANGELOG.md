...
- Usage bars are now coloured by **burn rate** instead of absolute level: red when the current pace projects past the limit before the window resets, yellow when it projects into the last 10%, green otherwise. A percentage on its own cannot say whether it is a problem — 39% of a weekly window is comfortable on day five and a blow-out on day two — and the old 70/90 thresholds painted exactly that blow-out green. Measured on two real accounts: one at 39% used with 4d20h left of the week now reads red (projected 127%), while another at 76% used with 1d16h left reads yellow (projected 100%). Window lengths come from `limit_window_seconds` for Codex; Anthropic's usage payload carries no duration field, so they are inferred from the limit group (`session` → 5h, `weekly*` → 7d), and an unrecognized limit keeps its absolute colour rather than guessing. The absolute thresholds also remain in force wherever no projection is trustworthy: no usable reset time, less than `max(60s, 1%)` of the window elapsed, or under 5% of the quota spent, where whole-percent resolution makes the projection noise. Colour is the whole change — no marker on the bar, no extra column, no layout change — so `--json` and every `color: false` rendering, golden fixtures included, are byte-for-byte unchanged (REQ-011, DEC-012).
...
- A dead seat profile no longer aborts turns that never touch it. `syncTurn()` looped over both providers and aborted the turn on any failure, with no reference to the model actually running, so an `invalid_grant` on the anthropic default killed every turn — including turns on `openai-codex`, `cursor/…`, `google/…` or any other provider needing no seat credential. That is exactly the escape hatch the design is supposed to protect: when a subscription dies you switch models and keep working, and instead the provider you switched away from blocked you. The abort is now scoped to `ctx.model?.provider`; everything else is unchanged — both providers still sync every turn, the failing one is still blocked and sentinel-poisoned, and the first turn that selects it still aborts byte-identically. A failure on an idle provider now reports itself as a non-fatal notice. When the active model cannot be identified, the paranoid default holds and any failure aborts (AC-031). The non-fatal path does not trust that judgement blindly: `ctx.model` is live agent state while the request rides a config snapshot taken before `turn_start`, so a turn survives an idle provider's failure only when the sentinel verifiably landed on it — an unlanded sentinel escalates back to an abort (AC-032).
- A usage limit whose reset timestamp cannot be parsed no longer breaks the report. The payloads are cast, not validated, so an unparsable `resets_at` (or a `reset_at` outside `Date` range) became an Invalid Date and was rendered as one: on a wide terminal `fmtReset` handed it to `Intl.DateTimeFormat`, which throws `RangeError` and took down the whole report, and on a narrow one it printed `in NaNm`. Such a timestamp is now dropped at the parse boundary, so it costs its own column and nothing else — the bar and percentage render normally and the meter keeps its absolute colour. This predates pace colouring; it surfaced while testing that a malformed timestamp cannot mis-colour a meter (AC-030).
